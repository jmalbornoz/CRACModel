...
% polynomial fit for specific heat capacity of water vapor, joule(kg-C)
poly = [0.0011 0.1915 1.8585e+003];
...
% specific heat capacity of waper vapour at return temperature, joules/(kg-C)
%c_vap = poly(1)*Tr^2 + poly(2)*Tr + poly(3);

% humidity ratio, kg H20/kg dry air
%HR = 0.62198*Pvap./Pdry;
    
% specific heat capacity as a function of water vapour content, Joule/(kg.C)
%c_ah = c_a + 1820*HR; 
...
if Tcoil > Tdew     %% dry coil operation
    ...
    % specific heat capacity of water vapour at supply temperature, joules/(kg-C)
    %c_vap = poly(1)*Ts^2 + poly(2)*Ts + poly(3);
    ...
    % specific heat capacity of water vapour at supply temperature, joules/(kg-K)
    %c_vap = poly(1)*Ts^2 + poly(2)*Ts + poly(3);
